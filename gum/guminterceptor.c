...
static GHashTable * rwx_pages = NULL;


//在 gum_interceptor_transaction_end 函数中记录被赋予 RWX 权限的内存区域，并在事务结束时回收这些内存区域的写权限。
static void
record_rwx_page (gpointer page)
{
  if (rwx_pages == NULL)
    rwx_pages = g_hash_table_new (g_direct_hash, g_direct_equal);

  g_hash_table_add (rwx_pages, page);
}

static void
revoke_rwx_permissions (void)
{
  if (rwx_pages == NULL)
    return;
  ...
  g_hash_table_iter_init (&iter, rwx_pages);
  while (g_hash_table_iter_next (&iter, &key, NULL))
  {
    LOGD("Revoking RWX permissions for page: %p", key);
    gum_mprotect (key, gum_query_page_size(), GUM_PAGE_RX);
  }

  g_hash_table_remove_all (rwx_pages);
}
...
gum_interceptor_transaction_end (GumInterceptorTransaction * self)
{
  ...
  // LOGD("Revoking RWX permissions after hook is applied.");
  // revoke_rwx_permissions();

no_changes:
  gum_interceptor_unignore_current_thread (interceptor);

  // 延迟收回内存的写权限，直到所有自定义回调注册完毕
  LOGD("Revoking RWX permissions after hook is applied.");
  revoke_rwx_permissions();
}
